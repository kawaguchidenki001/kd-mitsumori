...
| 対象 | 丸め |
|---|---|
| 単価 | **有効桁で切上げ**：1,000円以上は**上3桁**（4桁目を繰り上げ）／100〜999円は**上2桁**／10〜99円は**上1桁**。掛率を掛けた後も必ずこの形にする |
| プルボックス | 100円単位（切上げ） |
| 電工費・高所作業車損料などの労務・機械の1式 | 1,000円単位（切上げ） |
| 分類（工事箇所・棟）ごとの小計 | **1,000円単位**（切捨て）。端数は消耗雑材費など最後の1式行で調整する |
| 法定福利費・運搬費など**経費関係**の各行 | **1,000円単位**（切捨て）。集計行の `adj` で吸収する。小計調整用の1式行（消耗雑材費など）も1,000円単位調整を優先し、上記の有効桁ルールは適用しない |
| **計（税抜）** | **1,000円単位**（切捨て）。端数は**最後の諸経費行**の `adj` で吸収する（この行だけ1,000円単位でなくてよい） |

```python
def sig(v):   # 単価の丸め（1,000円以上=上3桁／100円台=上2桁／10円台=上1桁、いずれも切上げ）
    import math
    v = float(v)
    if v <= 0: return 0
...
# 例 386,750→387,000／13,420→13,500／967→970／55→60
```
