...
# スイッチ配線・コンセント配線の単価には、器具の費用と取付も含む。
T+=it("電灯配線","",NFIX+1,"ケ所",2800,f"器具1台あたり配線一式。器具{NFIX}台＋ユニットバス照明1ケ所")
# 換気扇（機器リスト P-1）：F-1 レンジフード（キッチン工事で別途）1F:K／F-2 パイプファン天井付 1F:WC／
#   F-3 パイプファン壁付 2F:WC／F-4 浴室暖房換気（ユニットバスのオプション）1F:浴室。配線は4台とも当方。
T+=it("換気配線","",4,"ケ所",3600,"F-1〜F-4（F-4は浴暖リモコン線）。No.260114の実績単価")
T+=it("スイッチ配線","片切",N_SW1,"ケ所",4200,"スイッチ・取付共。No.260114の実績単価")
T+=it("スイッチ配線","3路",N_SW3,"ケ所",5500,"スイッチ・取付共。★片切4,200からの積上げ")
T+=it("スイッチ配線","4路",N_SW4,"ケ所",6500,"スイッチ・取付共。★片切4,200からの積上げ")
T+=it("スイッチ配線","換気用",N_SWF,"ケ所",6500,"1F・2F WC の消し遅れスイッチ（F-2・F-3）・取付共。No.260114の実績単価")
T+=it("スイッチ配線","浴室照明",1,"ケ所",4200,"サニタリー側。スイッチ・取付共")
...
T+=it("コンセント配線","2口",N_CN2,"ケ所",3400,"コンセント・取付共。No.260114の実績単価")
T+=it("コンセント配線","接地極付",N_CNE,"ケ所",4800,"コンセント・取付共。No.260114のEET単価")
T+=it("コンセント配線","防水",N_CNW,"ケ所",5200,"コンセント・取付共。No.260114の実績単価")
T+=it("コンセント配線","エアコン専用",N_CNA,"ケ所",14000,"コンセント・取付共。No.260114の専用回路単価")
T+=it("コンセント配線","200V専用",N_CN200,"ケ所",14500,"コンセント・取付共。No.260114の専用・防水単価")
T+=it("コンセント配線","給湯器用",1,"ケ所",14500,"屋外給湯器の専用電源（防水）。図面に給湯器リモコンあり。No.260114の専用・防水単価")
T+=it("専用回路配線","浴室暖房乾燥機",1,"ケ所",14000,"F-4 浴室暖房換気の専用電源。No.260114の専用回路単価")
T+=it("回路配線","分電盤〜各回路",N_CIR,"ケ所",4800,f"No.260114の実績単価。{N_CIR}回路")
...
# ===== 03 換気設備（取付）=====
# F-1 レンジフード本体・取付はキッチン工事（別途）、屋外フードは建築工事。
# F-4 浴室暖房換気の本体はユニットバスのオプション（UB工事で取付）。ダクト・ベントキャップは当方。
cat("03 換気設備")
FAN = q('換気扇(浴室用)','風圧式 13cm用 引きひもなし')      # 18,600（パイプファンの登録なし）
VC  = q('ウェザーカバー(浴室用)','プラスチック製 13cm用')     # 9,510（φ100ベントキャップの登録なし）
T+=it("換気扇 Ｆ－２","パイプファン 天井付",1,"台",FAN,"1F:WC。φ100・70m3/h。単価：換気扇(浴室用)13cmで代用")
T+=it("換気扇 Ｆ－３","パイプファン 壁付",1,"台",FAN,"2F:WC。φ100・70m3/h。単価：換気扇(浴室用)13cmで代用")
T+=it("排気ダクト","φ100",3,"ケ所",6000,"★F-2・F-3・F-4。ダクト・継手・支持・取付共")
T+=it("ベントキャップ","φ100",3,"個",VC,"F-2・F-3・F-4。単価：ウェザーカバー(浴室用)13cmで代用")
