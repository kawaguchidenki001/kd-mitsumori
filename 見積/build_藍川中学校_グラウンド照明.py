...
"""藍川中学校グラウンド照明設備工事（岐阜市長 柴橋正直 御中）
   ...
   数量・金額は R8.9.4 いただいた内訳（投光器6台・メタハラ削除・取替工料178,000・
   撤去品処分費21,000・諸経費65,150）に合わせてある。日付は令和8年9月4日。
...
sub = 0
sub += it("LED投光器",      "LD-240D",       6, "台", 32_000)
sub += it("落下防止ワイヤー", "JD-004K",       6, "個",    900)
sub += it("アイボルト",      "SUS　M12",      6, "個",  1_100)
sub += it("リングキャッチ",   "SUS　M6",       6, "個",    550)
sub += it("投光器取付架台",   "PDP2221",       3, "組", 11_100)
sub += it("取付台座",       "TFB-13901N",    6, "個",  7_200)
sub += it("ケーブル",       "CVV2sq-3c",    10, "ｍ",    250)
sub += it("電線管",        "支持材含む",       1, "式",  8_750)
sub += it("雑材・消耗品",     "",             1, "式",  9_800)
sub += it("取替工料",       "",              1, "式", 178_000, 1.00)   # 全額が労務費
sub += it("高所作業車使用",   "",              1, "日",  37_000)
sub += it("撤去品処分費",     "",             1, "式",  21_000)

# 諸経費：純工事費×12％＋端数調整（計（税抜）を1,000円単位に）
KEIHI_RATE = 12.0
k_amt = 65_150
...
assert (sub, net, net + tax) == (540_850, 606_000, 666_600), (sub, net)
