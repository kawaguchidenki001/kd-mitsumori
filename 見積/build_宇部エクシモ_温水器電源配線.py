...
"""宇部エクシモ温水器電源配線工事（戸島工業株式会社 御中）
   単価は「電気設備工事積算実務マニュアル2026（岐阜県）」の複合単価（材料費＋労務費＋経費）。
   メタルモールは同マニュアルの正式品名「1種金属線ぴ(MM1) A型 定尺」で引くこと
   （商品名「メタルモール」で探すとワイヤプロテクタ＝合成樹脂モールを拾うので注意）。
   複合単価表に無いもの（線ぴ用コーナーボックス・ELB2P1E20A増設・器具脱着）だけを
   材料実勢＋マニュアルの手間から積み上げ、note に★を付けた。
"""
...
s += it("メタルモール", "A型 1種金属線ぴ(MM1) 定尺", 2.5, "m", 3_680, 1_974, "複合単価（1種金属線ぴ(MM1) A型 定尺）")
s += it("コーナーボックス", "A型用", 1, "個", sig(255 + 2_820 + 1_357), 2_820, "★材料255＋線ぴ用ボックス取付手間")
s += it("スイッチボックス", "1個用 浅形", 1, "個", 5_050, 2_820, "複合単価")
