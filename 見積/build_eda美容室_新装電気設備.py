...
"""eda美容室新装電気設備工事（株式会社廣瀬住建 御中）
   R8.7.29 発行の紙見積（No.260725）を、そのままKD見積へ置き換えたもの。
   金額は一切変更していない（小計880,000／消費税88,000／合計968,000）。
...
net += it("コンセント",       "2口",                  8, "箇所", 4_500, 0.60)
net += it("専用回路コンセント", "",                     5, "箇所", 11_000, 0.65)
net += it("片切スイッチ",     "",                    10, "箇所", 4_800, 0.60)
net += it("3路スイッチ",      "",                     2, "箇所", 6_800, 0.60)
net += it("回路配線",         "",                     4, "回路", 6_500, 0.70)
net += it("配線ダクトレール",  "2m",                   4, "カ所", 13_000, 0.35)
net += it("配線ダクトレール",  "3m",                   2, "カ所", 19_500, 0.35)
