...
TOT = {}
def betsu(name, spec):
    """土工事など別途扱いの行（品名のみ表示・数量／単価／金額は空欄）"""
    rows.append({"name": name + "（別途）", "spec": spec, "qty": 0, "unit": "　",
                 "price": 0, "note": "土工事につき別途"})

t = "基礎工"; cat(t); s = 0
betsu("基礎掘削及びスパイラルダクト立込", "φ500 2m以下")
s += it("スパイラルダクト", "φ500×t0.6×1700", 1, "本", sig(P["ダクト"] * 2), NOTE["ダクト"] + "／2倍")
betsu("基礎砕石", "RC-30 t=100")
betsu("基礎コンクリート", "24-8-25(20)高炉 小型構造物・人力打設")
s += it("アンカーフレーム", "4-M24-L600", 1, "組", sig(P["アンカー"] * 1.5), NOTE["アンカー"] + "／1.5倍")
s += it("接地設置工", "φ14×1500 D種接地3m以内（接地棒・低減剤共）", 1, "極", sig(16600 * 2), "2倍", 0.55)
betsu("根巻きコンクリート", "18-8-40BB 0.075m3・型枠0.6m2共")
TOT[t] = s
...
s += it("道路照明灯建柱", "350kg以下 建柱車", 1, "基", 85000, "ご指示額", 0.50)
