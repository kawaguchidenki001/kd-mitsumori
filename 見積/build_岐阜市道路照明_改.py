...
def up(v, unit=10): return int(math.ceil(round(v, 6) / unit)) * unit
MARKUP = 1.3
...
for k, (teika, r) in NET.items():
    ...
    P[k] = up(net * MARKUP, 10)
    NOTE[k] = f"富永電機No.9021 {teika:,}円×{r:.2f}＝NET {net:,}円 ×1.3"
...
TOT = {}
t = "基礎工"; cat(t); s = 0
s += it("基礎掘削及びスパイラルダクト立込", "φ500 2m以下", 1, "基", 11950)
s += it("スパイラルダクト", "φ500×t0.6×1700", 1, "本", P["ダクト"], NOTE["ダクト"])
s += it("基礎砕石", "RC-30 t=100", 0.2, "m2", 970, "10円単位に整理（旧 967円）")
s += it("基礎コンクリート", "24-8-25(20)高炉 小型構造物・人力打設", 0.3, "m3", 25000)
s += it("アンカーフレーム", "4-M24-L600", 1, "組", P["アンカー"], NOTE["アンカー"])
s += it("接地設置工", "φ14×1500 D種接地3m以内（接地棒・低減剤共）", 1, "極", 16600)
s += it("根巻きコンクリート", "18-8-40BB 0.075m3・型枠0.6m2共", 1, "箇所", 6000)
TOT[t] = s
...
s += it("道路照明灯建柱", "350kg以下 建柱車", 1, "基", 35780)
s += it("引込フックバンド", "", 1, "個", 5000)
s += it("管理銘板", "", 1, "枚", 3000)
TOT[t] = s
...
s += it("LED照明器具", "KCE150-3C（SLM4-X20830AW-VK00 電源装置内蔵）", 1, "組",
        P["LED"], NOTE["LED"])
s += it("照明器具取付", "新設（高所作業車共）", 1, "台", 25000)
s += it("専用ケーブル", "直線型ポール用", 1, "本", 8000)
s += it("ジョイントユニット", "テストスイッチ有", 1, "個", 12000)
s += it("電線", "VVF2.6mm×2C", 10, "m", 1270)
s += it("電線", "VVF1.6mm×3C", 10, "m", 970)
TOT[t] = s
...
assert all(r["price"] % 10 == 0 for r in d["rows"] if "qty" in r)
